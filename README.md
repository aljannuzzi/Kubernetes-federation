...
In this example, I will describe my experience on setting up two Kubernetes clusters on the Microsoft Azure cloud platform in different regions and configuring a federation among them. I will use Azure DNS as the external DNS for the federation and describe the hacks that made it work, as Azure is still not natively supported by the federation service as a DNS provider (it will be soon!)
...
In this example, I'm using bash on Windows10, under the fantastic Linux subsystem for Windows.
...
While this was running, I copied the k8.json file to k8cluster2.json, edited the location of the cluster to be in a different datacenter and run it again.

After both commands finished, I had my two kubernetes clusters provisioned on Azure.

## Making sure you can log into the Master node

One of your clusters will be the host cluster for the federation. 
In this case, I selected the wesuscluster1 to be the host cluster.

I will use westuscluster1 master node to be the place where kubectl will be used to manage the federation. It can be any other place where kubectl have credentials and works.

Just to make sure I can acess westuscluster1 master, I connected through ssh using the azureuser private key, stored in the _output directory after acs-engine finshes its execution.
...
I'll name the zone in this example "syncweek.com".
...
In this phase I followed [this awesome hack](https://github.com/xtophs/k8s-setup-federation-cluster) created by my good friend and Kubernetes ninja, Christoph.
...
In my case, it was 1.8.11. 

**Trick part: I should use "v1.8.11" in the URL.** 
...
After expanding the files, I copied kubefed to my /usr/local/bin folder.
...
In my case, I copyied the file this way:
...
I used [these beautiful yaml files](https://github.com/CSELATAM/istio-scenarios/blob/master/src/scenarios.yaml) created by my great friend Allan Targino for another project to test the deployment under the federation.

After downloading the files to my master node, I run the following command:
...
The federation conext doesn't have anything else deployed besides the Deployment. So, it is expected that kubectl get <reource> would return information only if we ask for "deployments".
